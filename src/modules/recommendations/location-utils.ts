import type {
  RecommendationLocationCoordinate,
...
type RecommendationLocationPayload =
  | Partial<RecommendationMustHavePayload>
  | Partial<RecommendationPreferencesPayload>
  | null
  | undefined;
...
export const getLocationNamesFromPayload = (
  payload: RecommendationLocationPayload,
): string[] =>
...
export const hasLocationCriteria = (
  payload: RecommendationLocationPayload,
): boolean => getLocationNamesFromPayload(payload).length > 0;

export const buildCoordinatesFromPlaces = (
  places: RecommendationPlaceDetails[],
): RecommendationLocationCoordinate[] =>
  places.map((place) => ({
...
export const serializeLocationCoordinate = (
  coordinate: RecommendationLocationCoordinate,
): string => `${coordinate.latitude},${coordinate.longitude}`;

export const parseLocationCoordinate = (
  value: string,
): RecommendationLocationCoordinate | null => {
